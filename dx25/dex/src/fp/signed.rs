...
pub mod concordium {
    ...
    pub trait ConditionalSerialisation: Serial + Deserial {}
    impl<T: Serial + Deserial> ConditionalSerialisation for T {}
}
...
#[cfg(feature = "multiversx")]
pub mod multiversx {
    // Need this qualification, because we have derive macros with the same names
    use multiversx_sc_codec::{NestedDecode, NestedEncode, TopDecode, TopEncode};

    pub trait ConditionalSerialisation:
        TopEncode + TopDecode + NestedDecode + NestedEncode
    {
    }
    impl<T: TopEncode + TopDecode + NestedDecode + NestedEncode> ConditionalSerialisation for T {}
}
...
#[cfg(feature = "concordium")]
pub use concordium::ConditionalSerialisation;

#[cfg(feature = "multiversx")]
pub use multiversx::ConditionalSerialisation;
...
impl<T> ops::SubAssign for Signed<T>
where
    T: ops::Add
        + ops::AddAssign
        + ops::SubAssign
        ...
        + ConditionalSerialisation,
{
    fn sub_assign(&mut self, mut rhs: Self) {
        rhs.non_negative = !rhs.non_negative;
        *self += rhs;
    }
}
...
impl<T> CheckedAdd for Signed<T>
where
    T: ops::Add
        + ops::Add<Output = T>
        + ops::Sub
        ...
        + CheckedSub,
{
    fn checked_add(&self, rhs: &Self) -> Option<Self> {
        if self.non_negative == rhs.non_negative {
            ...
            // Using checked_sub here because ops::Sub requires values (not references), and we want to avoid cloning.
            // Unwrap must always succeed as self.value >= rhs.value
            ...
            let non_negative = rhs.non_negative;
            Some(Self {
                value,
                non_negative,
            })
...
impl<T> CheckedMul for Signed<T>
where
    T: ops::Add
        + ops::Add<Output = T>
        + ops::Sub
        ...
        + CheckedMul,
{
    fn checked_mul(&self, v: &Self) -> Option<Self> {
        match self.value.checked_mul(&v.value) {
            Some(value) => Some(Self {
                value,
                non_negative: self.non_negative == v.non_negative,
            }),
            None => None,
        }
    }
