...
impl CheckedAdd for U256X320 {
    fn checked_add(&self, v: &Self) -> Option<Self> {
        self.0.checked_add(v.0).map(Self)
    }
}

impl CheckedSub for U256X320 {
...
mod test {
    ...
    #[test]
    fn test_sum() {
        let one = U256X320(U576::one());
        let two = U256X320(U576::one() * 2);
    ...
    #[test]
    fn test_mul() {
        let real_one = U256X320(U576::one() << 320);
        let real_two = U256X320((U576::one() << 320) * 2);
    ...
    fn test_mul_large() {
        assert_eq!(
            U256X320::from(1u128 << 100) * U256X320::from(1u128 << 26),
            U256X320::from(1u128 << 126)
        );
    }
    ...
    #[test]
    fn test_floor() {
        let unit = U576::one() << 256;
        let cases = [
            U256X320(unit * 2),
            U256X320(unit * 34141),
            U256X320(unit * 1_435_134_134),
            U256X320(unit * 1),
            U256X320((unit >> 2) + 111),
            U256X320((unit >> 34) + 33),
            U256X320(unit << 32),
        ...
        for x in cases {
            println!("Case: {}", x.0);
            assert_eq!(x.floor(), U256X320((x.0 >> 320) << 320));
        }
    }
    ...
    fn test_fract() {
        ...
        let cases = [
            ...
            U256X320((unit << 34) + 13),
            U256X320(unit << 32),
            U256X320((unit << 64) + 1231),
        ];
        for x in cases {
            println!("Case: {}", x.0);
            assert_eq!(x.fract(), U256X320((x.0 << 320) >> 320));
        }
    }

    #[test]
    fn test_integer_sqrt() {
        let four = U256X320::from(4);
        let two = U256X320::from(2);
        assert_eq!(four.integer_sqrt(), two);
    }

    #[test]
    fn test_ceil() {
        assert_eq!((U256X320::from(0)).ceil(), U256X320::from(0));
        assert_eq!(U256X320::from(237).ceil(), U256X320::from(237));
        assert_eq!(
            (U256X320::from(115) / U256X320::from(10)).ceil(),
            U256X320::from(12)
        );
        assert_eq!(
            (U256X320::from(723_000_000_001) / U256X320::from(1_000_000_000)).ceil(),
            U256X320::from(724)
        );
    }
}
