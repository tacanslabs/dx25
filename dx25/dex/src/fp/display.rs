...
impl fmt::Display for U320X192 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let value_rug = rug::Float::with_val(512, self.0 .0[7]) * &two_pow_256
            + rug::Float::with_val(512, self.0 .0[6]) * &two_pow_192
            + rug::Float::with_val(512, self.0 .0[5]) * &two_pow_128
            + rug::Float::with_val(512, self.0 .0[4]) * &two_pow_64
            + rug::Float::with_val(512, self.0 .0[3])
            + rug::Float::with_val(512, self.0 .0[2]) / &two_pow_64
            + rug::Float::with_val(512, self.0 .0[1]) / &two_pow_128
            + rug::Float::with_val(512, self.0 .0[0]) / &two_pow_192;
        write!(f, "{value_rug}")
    }
}
...
impl fmt::Display for U320X320 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let two_pow_64 = rug::Float::with_val(640, 1u128 << 64);
        let two_pow_128 = (&two_pow_64 * &two_pow_64).complete(640);
        let two_pow_192 = (&two_pow_128 * &two_pow_64).complete(640);
        let two_pow_256 = (&two_pow_192 * &two_pow_64).complete(640);
        let two_pow_320 = (&two_pow_256 * &two_pow_64).complete(640);
        let value_rug = rug::Float::with_val(640, self.0 .0[9]) * &two_pow_256
            + rug::Float::with_val(640, self.0 .0[8]) * &two_pow_192
            + rug::Float::with_val(640, self.0 .0[7]) * &two_pow_128
            + rug::Float::with_val(640, self.0 .0[6]) * &two_pow_64
            + rug::Float::with_val(640, self.0 .0[5])
            + rug::Float::with_val(640, self.0 .0[4]) / &two_pow_64
            + rug::Float::with_val(640, self.0 .0[3]) / &two_pow_128
            + rug::Float::with_val(640, self.0 .0[2]) / &two_pow_192
            + rug::Float::with_val(640, self.0 .0[1]) / &two_pow_256
            + rug::Float::with_val(640, self.0 .0[0]) / &two_pow_320;
        write!(f, "{value_rug}")
    }
}
impl fmt::Debug for U320X320 {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(self, f)
    }
}
