...
use crate::dex::{validate_protocol_fee_fraction, ErrorKind};
use crate::{ensure_here, error_here, AmountUFP};
...
mod concordium {
    /// FIXME: requires adjustment according to Concordium (de)serialization
    use concordium_std::{
        schema::SchemaType, Deletable, Deserial, DeserialWithState, HasStateApi, Serial,
    };

    pub trait PersistentBound: HasStateApi {}

    impl<T: HasStateApi> PersistentBound for T {}

    pub trait PersistentCollection<T: PersistentBound>:
        Serial + DeserialWithState<T> + Deletable
    {
    }

    impl<T: PersistentBound, U: Serial + DeserialWithState<T> + Deletable> PersistentCollection<T>
        for U
    {
    }
    /// Bounding trait for serializable values which fit into single data block
    pub trait Persistent: Serial + Deserial {}

    impl<T: Serial + Deserial> Persistent for T {}

    /// Bounding trait for serializable values which are used in public WASM APIs
    pub trait WasmApi: Serial + Deserial + SchemaType {}

    impl<T: Serial + Deserial + SchemaType> WasmApi for T {}
}
#[cfg(feature = "concordium")]
pub use concordium::{Persistent, PersistentBound, PersistentCollection, WasmApi};
...
mod multiversx {
    ...
    pub trait PersistentCollection<T: PersistentBound>: NestedEncode + NestedDecode {}

    impl<T: NestedDecode + NestedEncode, U: PersistentBound> PersistentCollection<U> for T {}
    /// Bounding trait for serializable values which fit into single data block
    pub trait Persistent: NestedDecode + NestedEncode {}

    impl<T: NestedDecode + NestedEncode> Persistent for T {}

    /// Bounding trait for serializable values which are used in public WASM APIs
    pub trait WasmApi: NestedDecode + NestedEncode + TypeAbi {}

    impl<T: NestedDecode + NestedEncode + TypeAbi> WasmApi for T {}
}
#[cfg(feature = "multiversx")]
pub use multiversx::{Persistent, PersistentBound, PersistentCollection, WasmApi};
...
pub trait Types: Sized {
    ...
    type ContractExtraV1: PersistentCollection<Self::Bound> + Default;
...
pub trait ItemFactory<T: Types + ?Sized> {
    ...
    fn new_contract(
        ...
        Ok(Contract::V1(ContractLatest {
            ...
            protocol_fee_fraction: validate_protocol_fee_fraction(protocol_fee_fraction)
