...
mod tests {
    ...
    #[test]
    fn error_desc_roundtrip() {
        let error = error_here!(ErrorKind::AccountNotRegistered);

        let desc0 = ErrorDesc::from(&error);
        let code = i32::from(desc0);
        let desc1 = ErrorDesc::try_from(code).unwrap();

        assert_eq!(desc0, desc1);
    }
}
